    ...
    for(int k=0;k<=2;k++)
    {
      for(int l=0;l<=2;l++)
      {
        for(int m=0;m<=2;m++)
        {
          ...
          {
            tot+=d_prime_1*d_prime_2*d_prime_3*d_prime_4*zer_ab(2);
          }
...
void create_ov_mat_z(arma::mat &overlap, Shell** arr1)
{
    ...
    for (int i = 0; i < row_number; i++) 
    {
        ...
        {
          overlap(i,j)=sum_func_opt_z(arr1[i],arr1[j]);
...
//vnuc mat x
double vnuc_z(Shell* arr1,Shell* arr2)
{
  ...
  return V_nuc(2)*27.2114;
}


void v_nuc_z(arma::mat &v_nuc,Shell**arr1)
{
  ...
  for (int i = 0; i < row_number; i++) 
  {
    for (int j = 0; j < col_number; j++) 
    {
      ...
      {
        v_nuc(i,j)=vnuc_z(arr1[i],arr1[j]);
