...
//   Overlap_onedim(Ra(0), Rb(0), alphaa, alphab, la(0), lb(0));
...
double deriv_overlap_x(Shell &sh1, Shell &sh2)
{
    ...
    // double overlap_tot_der=overlap_x_der*overlap_y_der*overlap_z_der;
    // overlap_tot_der;
...
double normalize_func_y(Shell &sh1)
{
  double same_orb=integral_ov_y(sh1,sh1);
  same_orb=1/sqrt(same_orb);
  return same_orb;
}


double normalize_func_z(Shell &sh1)
{
  double same_orb=integral_ov_z(sh1,sh1);
  same_orb=1/sqrt(same_orb);
  return same_orb;
}
...
double overlap_int(Shell&sh1,Shell&sh2)
{
    double orb_1=normalize_func(sh1);
    double orb_2=normalize_func(sh2);
    double ov_elem=Eval_Ov(sh1,sh2);
    double ov_tot=sh1.get_d()*sh2.get_d()*orb_1*orb_2*ov_elem;
    return ov_tot;    
}



double sum_der_ov_x(Shell* arr1,Shell* arr2)
{
    double ov_tot_der=0;
    for(int i=0;i<=2;i++)
    {
        for(int k=0;k<=2;k++)
        {
            ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*(deriv_overlap_x(arr1[i],arr2[k])*integral_ov_y(arr1[i],arr2[k])*integral_ov_z(arr1[i],arr2[k]));
            // ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*normalize_func_y(arr1[i])*normalize_func_z(arr1[k])*(deriv_overlap_x(arr1[i],arr2[k])*integral_ov_y(arr1[i],arr2[k])*integral_ov_z(arr1[i],arr2[k]));
...
double sum_der_ov_y(Shell* arr1,Shell* arr2)
{
    double ov_tot_der=0;
    for(int i=0;i<=2;i++)
    {
        for(int k=0;k<=2;k++)
        {
            ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*(integral_ov_x(arr1[i],arr2[k])*deriv_overlap_y(arr1[i],arr2[k])*integral_ov_z(arr1[i],arr2[k]));
            // ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*normalize_func_y(arr1[i])*normalize_func_z(arr1[k])*(deriv_overlap_x(arr1[i],arr2[k])*integral_ov_y(arr1[i],arr2[k])*integral_ov_z(arr1[i],arr2[k]));
...
double sum_der_ov_z(Shell* arr1,Shell* arr2)
{
    double ov_tot_der=0;
    for(int i=0;i<=2;i++)
    {
        for(int k=0;k<=2;k++)
        {
            ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*(integral_ov_x(arr1[i],arr2[k])*integral_ov_y(arr1[i],arr2[k])*deriv_overlap_z(arr1[i],arr2[k]));
            // ov_tot_der+=arr1[i].get_d()*arr2[k].get_d()*normalize_func_y(arr1[i])*normalize_func_z(arr1[k])*(deriv_overlap_x(arr1[i],arr2[k])*integral_ov_y(arr1[i],arr2[k])*integral_ov_z(arr1[i],arr2[k]));
...
void create_ov_mat_der_x(arma::mat &overlap, Shell** arr1)
{
    ...
    for (int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
            if(i!=j)
            {
                overlap(i,j)=sum_der_ov_x(arr1[i],arr1[j]);
...
void create_ov_mat_der_y(arma::mat &overlap, Shell** arr1)
{
    double row_number=size(overlap)[0];
    double col_number= size(overlap)[1];
    for (int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
            if(i!=j)
            {
                overlap(i,j)=sum_der_ov_y(arr1[i],arr1[j]);
...
void create_ov_mat_der_z(arma::mat &overlap, Shell** arr1)
{
    double row_number=size(overlap)[0];
    double col_number= size(overlap)[1];
    for (int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
            if(i!=j)
            {
                overlap(i,j)=sum_der_ov_z(arr1[i],arr1[j]);
    ...
    for(int k=0;k<=2;k++)
    {
      for(int l=0;l<=2;l++)
      {
        for(int m=0;m<=2;m++)
        {
          ...
          double T_2=V_squared*pow(R_ab_dist,2);

          double U=Ua*Ub;
          double zer_AB=(U/pow(R_ab_dist,2))*(2*sqrt(V_squared)/sqrt(M_PI)*exp(-T_2)-erf(sqrt(T_2))/R_ab_dist)*R_ab_dist;
    ...
    for(int k=0;k<=2;k++)
    {
      for(int l=0;l<=2;l++)
      {
        for(int m=0;m<=2;m++)
        {
          ...
          arma::vec v_inside=2*sqrt(V_squared)/sqrt(M_PI)*exp(-T);
          arma::vec v_inside_2=erf(sqrt(T))/r_ab;
          

          double v_inside_x=2*sqrt(V_squared)/sqrt(M_PI)*exp(-T(0));
        //   cout << v_inside_x;
        //   cout << "\n\n";
          double v_inside_2_x=erf(sqrt(T(0)))/r_ab(0);
        //   cout << v_inside_2_x;
        //   cout << "\n\n";

          double uab_x= U/r_ab_(0);
        //   cout << U/r_ab_(0);
        //   cout << "\n\n";
          double zer_AB_x=(U/r_ab_(0))*(v_inside_x-v_inside_2_x)*r_ab(0);
          


        //   arma::vec zer_AB=(U/r_ab_)*(v_inside-v_inside_2)*r_ab;
        ...
        //   else
          {
            tot+=d_prime_1*d_prime_2*d_prime_3*d_prime_4*zer_AB_x;
          }
    ...
    for(int k=0;k<=2;k++)
    {
      for(int l=0;l<=2;l++)
      {
        for(int m=0;m<=2;m++)
        {
          ...
          double U=Ua*Ub;
          double v_inside_y=2*sqrt(V_squared)/sqrt(M_PI)*exp(-T(1));
          cout << v_inside_y;
          cout << "\n\n";
          double v_inside_2_y=erf(sqrt(T(1)))/r_ab(1);
          cout << v_inside_2_y;
          cout << "\n\n";

          double uab_y= U/r_ab_(1);
          cout << U/r_ab_(0);
          cout << "\n\n";

          double zer_AB_y=(U/r_ab_(1))*(v_inside_y-v_inside_2_y)*r_ab(1);
        ...
        //   else
          {
            tot+=d_prime_1*d_prime_2*d_prime_3*d_prime_4*zer_AB_y;
          }
...
double gamma_func_z(Shell* arr1,Shell* arr2)
{
  double tot=0;
  for(int i=0;i<=2;i++)
  {
    for(int k=0;k<=2;k++)
    {
      for(int l=0;l<=2;l++)
      {
        for(int m=0;m<=2;m++)
        {
          ...
          double v_inside_z=2*sqrt(V_squared)/sqrt(M_PI)*exp(-T(2));
          cout << v_inside_z;
          cout << "\n\n";
          double v_inside_2_z=erf(sqrt(T(2)))/r_ab(2);
          cout << v_inside_2_z;
          cout << "\n\n";

          double uab_z= U/r_ab_(2);
          cout << U/r_ab_(2);
          cout << "\n\n";

          double zer_AB_z=(U/r_ab_(2))*(v_inside_z-v_inside_2_z)*r_ab(2);
        ...
        //   else
          {
            tot+=d_prime_1*d_prime_2*d_prime_3*d_prime_4*zer_AB_z;
          }
...
void create_gamma_mat_z(arma::mat &gamma, Shell** arr1)
{
    double row_number=size(gamma)[0];
    double col_number= size(gamma)[1];
    for (int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
            if(i!=j)
            {
                gamma(i,j)=gamma_func_z(arr1[i],arr1[j]);
            }
...
double scf_real_v_nuc_x(double n_atom, Shell** arr1)
{
  double V_nuc=0;
  for (int i = 0; i < n_atom; i++)
  {
    for (int j = 0; j < i; j++) 
    {
      ...
      arma::vec r_ab=Ra-Rb;    
      arma::vec r_ab_=pow(r_ab,3);
      double R_ab_dist=sqrt(r_ab_(0)+r_ab_(1)+r_ab_(2));
      V_nuc+=(-1*arr1[i][0].get_elem_num()*arr1[j][0].get_elem_num())*r_ab(0)/r_ab_(0);
...
double scf_real_v_nuc_y(double n_atom, Shell** arr1)
{
  double V_nuc=0;
  for (int i = 0; i < n_atom; i++)
  {
    for (int j = 0; j < i; j++) 
    {
      ...
      arma::vec r_ab_=pow(r_ab,3);
      double R_ab_dist=sqrt(r_ab_(0)+r_ab_(1)+r_ab_(2));
      V_nuc+=(-1*arr1[i][0].get_elem_num()*arr1[j][0].get_elem_num())*r_ab(1)/r_ab_(1);
...
double scf_real_v_nuc_z(double n_atom, Shell** arr1)
{
  double V_nuc=0;
  for (int i = 0; i < n_atom; i++)
  {
    for (int j = 0; j < i; j++) 
    {
      ...
      arma::vec r_ab_=pow(r_ab,3);
      double R_ab_dist=sqrt(r_ab_(0)+r_ab_(1)+r_ab_(2));
      V_nuc+=(-1*arr1[i][0].get_elem_num()*arr1[j][0].get_elem_num())*r_ab(2)/r_ab_(2);
...
void create_h_core_mat_x(arma::mat &hcore,arma::mat gamma2_off,Shell** arr1)
{
    ...
    for (int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
            ...
            //   arr1[i][0].get_gamma()-(arr1[i][0].get_elem_num()-0.5)*sum(gamma3_diag.row(j))-sum(gamma2_off.row(j));
              // -(arr1[j][0].get_elem_num()*gamma_func(arr1[i],arr1[j]));
            }
            else
            {
              hcore(i,j)=0.5*(arr1[i][0].get_beta()+arr1[j][0].get_beta())*sum_der_ov_x(arr1[i],arr1[j]);
            }
...
//pff diag gamma
void gamma_ptot_not_a_x(arma::mat &gamma,Shell** arr1,Shell** arr2)
{
  double row_number=size(gamma)[0];
  double col_number=size(gamma)[1];
  ...
  for(int i = 0; i < row_number; i++) 
    {
        for (int j = 0; j < col_number; j++) 
        {
          ...
          else
          {
            // cout << row_number/2;
            // cout << "\n\n";
            gamma(i,j)=((row_number/2)-arr2[j][0].get_elem_num())*gamma_func(arr1[i],arr2[j]);
          }
...
void g_mat_x(arma::mat &gmat,arma::mat gamma2_off,arma::mat pmat,Shell** arr1)
{
  ...
  for (int i = 0; i < row_number; i++) 
  {
    for (int j = 0; j < col_number; j++) 
    {
      ...
      {
        // gmat(i,j)=0;
        gmat(i,j)=arr1[j][0].get_gamma()*sum_func_opt(arr1[i],arr1[j])-pmat(i,j)*gamma_func(arr1[i],arr1[j]);
...
void g_mat_y(arma::mat &gmat,arma::mat deriv_overlap,arma::mat gamma2_off,arma::mat gamma3_diag,arma::mat pmat,Shell** arr1)
{
  double row_number=size(gmat)[0];
  double col_number= size(gmat)[1];
  for (int i = 0; i < row_number; i++) 
  {
    for (int j = 0; j < col_number; j++) 
    {
      ...
      {
        // gmat(i,j)=0;
        gmat(i,j)=arr1[j][0].get_gamma()*deriv_overlap(i,j)-pmat(i,j)*gamma_func(arr1[i],arr1[j]);
...
double scf_real_v_nuc(double n_atom, Shell** arr1)
{
  ...
  for (int i = 0; i < n_atom; i++)
  {
    for (int j = 0; j < i; j++) 
    {
      ...
      arma::vec r_ab_=pow(r_ab,2);
      double R_ab_dist=sqrt(r_ab_(0)+r_ab_(1)+r_ab_(2));
      V_nuc+=-1*(arr1[i][0].get_elem_num()*arr1[j][0].get_elem_num())/pow(R_ab_dist,3);
...
//ov - x 
  ...
  create_ov_mat_der_x(overlap_x,arr2);
  overlap_x.print();
  cout << "\n\n";

//ov - y 
  arma::mat overlap_y(10,10,arma::fill::zeros);
  create_ov_mat_der_y(overlap_y,arr2);
  overlap_y.print();
  cout << "\n\n";


//ov - z
  arma::mat overlap_z(10,10,arma::fill::zeros);
  create_ov_mat_der_z(overlap_z,arr2);
  overlap_z.print();
  cout << "\n\n";
...
//gamma - x 
  ...
  //gamma ab * zb x dir
  arma::mat gamma_ab(10,4,arma::fill::zeros);
  hcore_sum_not_x(gamma_ab,arr2,arr1);
  // gamma_ab.print();
  cout << "\n\n";
  
  //hcore_x
  arma::mat hcore_x(10,10,arma::fill::zeros);
  create_h_core_mat_x(hcore_x,gamma_ab,arr1);
  hcore_x.print();
  cout << "\n\n";
